function [ params ] = DoubletParamGuess( Lambda, Signal, varargin )
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here

    %Enforce defaults if needed
    if nargin < 3
        measurementType='T';
    else
    ...
    lambdaFWHMPoints=sort(lambdaFWHMPoints);
    ...
    if strcmpi(response,'No')
        close(fig)
        DoubletParamGuess(Signal, Lambda, measurementType);
    else
        close(fig)
    end
