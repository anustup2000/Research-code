...
def main(args):
    ...
    w= 13.5        # radius of the microdisk (in microns)
    Angle=57
    ...
    sz= c + h + 2*pad + 2*dpml # height of the computational cell
   ...
   #geometry=[mp.Cone(center= mp.Vector3(0,0,0),height= h,radius= w-h*np.sin(taperAngle),radius2= w+h*np.sin(taperAngle),axis= mp.Vector3(0,0,1),material= mp.Medium(index= n))]
    ...
    # output fields for one period at the end. (if we output at a single time, we might accidentally catch the E-z field when its nearly zero, and get a distorted field view). The data will be outputted in a rby t format. Output would be from -sr to sr (instead of 0 to sr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-fcen',type=float,default=0.62089,help='pulse center frequency')
    parser.add_argument('-df',type=float,default=0.009,help='pulse frequency width')
    parser.add_argument('-m',type=int,default=63,help='order of mode')
